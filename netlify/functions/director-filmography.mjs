...
export default async request=>{
  ...
  try{
    let person;
    if(id){person=await tmdb(`/person/${id}`,{language:KINOSIS_LOCALE.language});}
    else{
    ...
    const credits=await tmdb(`/person/${person.id}/movie_credits`,{language:KINOSIS_LOCALE.language});
    const byId=new Map();
    ...
    if(include.length){
      ...
      const extra=await pool(include.filter(movieId=>!existing.has(movieId)&&!exclude.has(movieId)),4,async movieId=>tmdb(`/movie/${movieId}`,{language:KINOSIS_LOCALE.language}));
      directed.push(...extra);
    }
