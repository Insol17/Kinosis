...
const search = fs.readFileSync('assets/js/features/search.js', 'utf8');
const collectionSearch = fs.readFileSync('assets/js/features/collection-search.js', 'utf8');
const collectionEditor = fs.readFileSync('assets/js/features/collection-editor.js', 'utf8');
...
const arthouseDirectory = fs.readFileSync('assets/js/features/arthouse-directory.js', 'utf8');
const directorProfileController = fs.readFileSync('assets/js/features/director-profile-controller.js', 'utf8');
...
const discoverCurationCarousel = fs.readFileSync('assets/js/features/discover-curation-carousel.js', 'utf8');
...
for (const importPath of ['./core/store.js','./core/router.js','./core/request-scheduler.js','./domain/personal-state.js','./domain/personal-actions.js','./domain/demo-state.js','./domain/auth-role.js','./infrastructure/api-client.js','./infrastructure/movie-repository.js','./services/movie-loader.js','./features/search.js','./features/collection-search.js','./features/collection-editor.js','./features/detail.js','./features/library.js','./features/arthouse.js','./features/arthouse-directory.js','./features/director-profile-controller.js','./features/discovery.js','./features/discover-curation-carousel.js','./features/studio.js','./features/calendar.js','./ui/movie-card.js']) {
  assert.ok(app.includes(`from '${importPath}'`), `app composition missing ${importPath}`);
}
assert.ok(html.includes('<script type="module" src="./assets/js/app.js?v=0.4.6.6"></script>'), 'browser entry must be a native ES module');
for (const legacy of ['features/search.js?v=0.4.6.6"></script>','core/movie-entities.js?v=0.4.6.6"></script>','services/movie-loader.js?v=0.4.6.6"></script>','features/detail.js?v=0.4.6.6"></script>']) assert.ok(!html.includes(`<script src="./assets/js/${legacy}`), `critical module still loaded as ordered global script: ${legacy}`);
...
assert.ok(search.includes("from './movie-search-index.js'") && collectionSearch.includes("from './movie-search-index.js'"), 'global and collection search must share the pre-indexed lookup utility');
assert.ok(!collectionEditor.includes('document.') && !collectionEditor.includes('fetch('), 'collection editor commands must remain UI/network agnostic');
...
assert.ok(!arthouseDirectory.includes('document.') && !arthouseDirectory.includes('fetch('), 'Arthouse directory renderer must stay DOM/network agnostic');
assert.ok(directorProfileController.includes('IntersectionObserver') && directorProfileController.includes('batchSize = 10') && !directorProfileController.includes('fetch('), 'Director profile controller must lazy-batch viewport enrichment through the repository');
...
assert.ok(discoverCurationCarousel.includes('IntersectionObserver') && !discoverCurationCarousel.includes('fetch('), 'Discover curation carousel may own DOM timing but must not own network access');
