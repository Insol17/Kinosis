...
for (const importPath of ['./core/store.js','./core/router.js','./domain/personal-state.js','./domain/personal-actions.js','./infrastructure/api-client.js','./infrastructure/movie-repository.js','./services/movie-loader.js','./features/search.js','./features/detail.js','./features/library.js','./ui/movie-card.js']) {
  assert.ok(app.includes(`from '${importPath}'`), `app composition missing ${importPath}`);
}
assert.ok(html.includes('<script type="module" src="./assets/js/app.js?v=0.4.5"></script>'), 'browser entry must be a native ES module');
for (const legacy of ['features/search.js?v=0.4.5"></script>','core/movie-entities.js?v=0.4.5"></script>','services/movie-loader.js?v=0.4.5"></script>','features/detail.js?v=0.4.5"></script>']) assert.ok(!html.includes(`<script src="./assets/js/${legacy}`), `critical module still loaded as ordered global script: ${legacy}`);
