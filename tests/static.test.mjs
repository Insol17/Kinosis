...
const required = [
  ...
  'assets/js/features/search.js','assets/js/features/detail.js','assets/js/features/library.js','assets/js/features/arthouse.js','assets/js/ui/movie-card.js','assets/js/cloud.js','assets/js/ui.js','assets/js/state-integrity.js','assets/js/curations.js','assets/js/curation-loader.js',
...
assert.ok(html.includes('app.js?v=0.4.5.1'), '0.4.5.1 cache-busting missing');
...
assert.ok(!html.includes('data-my="diary"'), 'Diary must remain inside Profile records');
...
assert.ok(app.includes('reviewArchiveHtml') && app.includes('data-my-review-archive'), 'Profile comment archive drill-down missing');
assert.ok(app.includes('curationRail') && app.includes('DIRECTOR ARCHIVE') && !app.includes('Director’s Archive :'), 'Arthouse curation rails must avoid duplicated archive labels');
...
assert.equal(curations.version, '0.4.5.1');
...
assert.ok(app.includes('arthouseCuratedMovies') && app.includes('programmedIds'), 'Arthouse curated source pool/deduplication missing');
assert.ok(!app.includes('WHERE TO WATCH</p><h2>어디서 볼 수 있나요'), 'conversational Detail availability copy regressed');
console.log('static.test: 0.4.5.1 Arthouse + Library + Detail + Profile polish contracts OK');
