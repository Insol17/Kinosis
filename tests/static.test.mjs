...
const required=[
  ...
  'netlify/functions/movie-search.mjs','netlify/functions/movie-detail.mjs','netlify/functions/movie-recommendations.mjs','netlify/functions/person-films.mjs','netlify/functions/director-filmography.mjs','netlify/functions/watchlist-availability.mjs','netlify/functions/supabase-health.mjs',
...
assert.ok(html.includes('app.js?v=0.4.4'),'asset cache-busting version missing');
assert.ok(html.includes('data/curations.js?v=0.4.4'),'curation data bundle missing');
assert.ok(html.includes('data-view="curation"'),'curation detail view missing');
for(const marker of ['/api/movie-search','/api/movie-recommendations','/api/person-films','/api/watchlist-availability','popstate','pushState','openCalendarDay','deleteLog','logEntryId','handleLetterboxdFiles','renderArthouse','viewingTimeline','refreshWatchlistAvailability','loadForYou','openCuration','renderCurationPage','fromCuration','renderHeroCarousel','pullCloudState','mergeCloudStates','/api/director-filmography'])assert.ok(js.includes(marker),`app missing ${marker}`);
assert.ok(!js.includes('renderAdmin'),'Admin UI logic should not ship in 0.4.4');
...
assert.ok(css.includes('grid-auto-columns:calc((100% - (6 * var(--poster-gap)))/7)'),'seven-card rail rule missing');
...
console.log('static.test: 0.4.4 routing, Reviews, Arthouse density, personalization, portability and file-curation surfaces OK');
