...
const required=[
  'index.html','assets/css/app.css','assets/css/editorial-0441.css','assets/js/app.js','assets/js/cloud.js','assets/js/ui.js','assets/js/config.js','assets/js/art-classifier.js','assets/js/importers.js','assets/js/curations.js',
...
for(const rel of ['sw.js','manifest.webmanifest','assets/js/recommender.js']) assert.ok(!fs.existsSync(path.join(root,rel)),`${rel} should be removed in 0.4.4.1`);
...
assert.ok(html.includes('app.js?v=0.4.4.1'),'asset cache-busting version missing');
assert.ok(html.includes('data/curations.js?v=0.4.4.1'),'curation data bundle version missing');
assert.ok(html.includes('editorial-0441.css?v=0.4.4.1'),'editorial design layer missing');
...
const editorial=fs.readFileSync(path.join(root,'assets/css/editorial-0441.css'),'utf8');
assert.ok(css.includes('grid-auto-columns:calc((100% - (6 * var(--poster-gap)))/7)'),'seven-card rail base rule missing');
assert.ok(editorial.includes('.film-masthead'),'new film detail design missing');
assert.ok(editorial.includes('.watch-options'),'Where to Watch redesign missing');
assert.ok(editorial.includes('content-visibility:auto'),'render containment/performance rule missing');
assert.ok(editorial.includes('backdrop-filter:none'),'expensive shell blur override missing');
...
console.log('static.test: 0.4.4.1 P0 integrity, performance, detail/watch UX and editorial redesign markers OK');
