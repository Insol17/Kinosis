...
const freshStamp = new Date().toISOString().slice(0, 10);
const item = { slug: 'kiarostami', source: { type: 'director', personId: '119294', snapshot: [{ id: '9', title: 'Snapshot' }], snapshotGeneratedAt: freshStamp } };
...
const fresh = await loader.ensure(item);
assert.equal(fresh.status, 'ready');
assert.equal(fresh.skipped, 'fresh-snapshot');
assert.equal(fetchCount, 0, 'fresh portfolio snapshot must not live-refresh on every visit');
const forced = await loader.retry(item);
assert.equal(forced.status, 'ready'); assert.equal(fetchCount, 1); assert.equal(forced.rows.length, 2); assert.equal(loader.state(item.slug).source, 'live');
...
const concurrentItem = { slug: 'erice', source: { type: 'director', snapshot: [{ id: '7' }], snapshotGeneratedAt: '2020-01-01' } };
...
const resilientItem = { slug: 'petzold', source: { type: 'director', snapshot: [{ id: '21', title: 'Snapshot Petzold' }], snapshotGeneratedAt: '2020-01-01' } };
...
console.log('curation-loader.test: fresh-snapshot TTL + state machine + retry + request dedupe OK');
