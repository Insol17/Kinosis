...
export function renderDetailHero(record, c) {
  ...
  const libraryAction = membership
    ? `<button data-remove-library="${c.escapeHtml(record.id)}">라이브러리에서 제거</button>`
    : `<button data-add-library="${c.escapeHtml(record.id)}">라이브러리에 추가</button>`;
  ...
  return `<section class="detail-hero ${backdropUrl ? '' : 'has-no-backdrop'} ${isLoading ? 'is-partial' : ''}" data-detail-part="hero">
    ...
    <div class="detail-hero-inner">
      ...
      <div class="detail-intro">
        ...
        <div class="detail-actions"><button class="primary-button detail-action" data-action="log" data-id="${c.escapeHtml(record.id)}">${logs.length ? '감상 기록 추가' : '감상 기록'}</button><button class="detail-watchlist ${relationship?.watchlist ? 'is-active' : ''}" data-action="watchlist" data-id="${c.escapeHtml(record.id)}">${relationship?.watchlist ? '✓ 보고싶어요' : '＋ 보고싶어요'}</button><details class="film-more"><summary class="detail-more" aria-label="더 보기">${c.icon('more')}</summary><div class="film-more-menu"><button class="${relationship?.favorite ? 'is-active' : ''}" data-action="favorite" data-id="${c.escapeHtml(record.id)}">${relationship?.favorite ? '♥ 좋아요 해제' : '♡ 좋아요'}</button><button data-action="collection-add" data-id="${c.escapeHtml(record.id)}">＋ 컬렉션에 추가</button>${libraryAction}${relationship || logs.length ? `<button class="is-danger" data-delete-personal-movie="${c.escapeHtml(record.id)}">모든 개인 데이터 삭제…</button>` : ''}</div></details></div>
      </div>
      <aside class="detail-relationship" aria-label="내 영화 평가">
        ${signedIn(c) ? `${c.starRatingHtml(record.id, relationship?.rating ?? null, 'detail')}<div class="detail-comment"><div class="detail-comment-head"><span>내 한줄평</span><button data-edit-relationship="${c.escapeHtml(record.id)}">${comment ? '수정' : '작성'}</button></div>${comment ? `<p>${c.escapeHtml(comment)}</p>` : '<p class="is-empty">이 영화에 대한 한줄평을 남겨보세요.</p>'}</div>` : '<button class="streaming-signin compact" data-open-auth>로그인하고 평가하기</button>'}
      </aside>
    </div>
  </section>`;
}
...
export function renderDetailAvailability(record, c) {
  return `<div class="detail-watch-band" data-detail-part="availability">${c.watchAvailabilityHtml(record)}</div>`;
}
...
export function renderDetailMetadata(record, c) {
  ...
  if (record.metadataLoading) {
    return `<div data-detail-part="metadata"><section class="detail-section detail-metadata-loading" role="status"><span class="loading-ring mini"></span><div><h2>작품 정보를 불러오는 중입니다.</h2><p>감독·출연·러닝타임은 준비되는 대로 이 영역만 갱신됩니다.</p></div></section></div>`;
  }
  if (record.detailError) {
    return `<div data-detail-part="metadata"><section class="detail-section detail-metadata-error" role="status"><div><h2>일부 작품 정보를 불러오지 못했습니다.</h2><p>${c.escapeHtml(record.detailError)} 이미 확인된 제목·포스터·내 기록은 그대로 사용할 수 있습니다.</p></div><button class="secondary-button" data-detail-retry="${c.escapeHtml(record.id)}">다시 불러오기</button></section></div>`;
  }
  return `<div data-detail-part="metadata">
    <section class="detail-section detail-synopsis"><h2>줄거리</h2><p>${c.escapeHtml(record.overview || '줄거리 정보가 없습니다.')}</p></section>
    ${record.director ? `<section class="detail-section detail-director-section"><div class="detail-section-head"><h2>감독</h2></div><div class="detail-director-card"><span class="cast-avatar-fallback">${c.escapeHtml(String(record.director || '?').slice(0, 1))}</span><div><strong>${director}</strong><small>${record.year || ''}${record.runtime ? ` · ${c.escapeHtml(c.fmtRuntime(record.runtime))}` : ''}</small></div></div></section>` : ''}
...
export function renderDetailActivity(record, c) {
  const relationship = c.relationship || c.entry || null;
  const logs = c.logs || [];
  if (!signedIn(c)) return `<section class="detail-section detail-my-record" data-detail-part="activity"><div class="detail-section-head"><h2>내 기록</h2></div><button class="streaming-signin compact" data-open-auth>로그인하고 감상 기록 남기기</button></section>`;
  return `<section class="detail-section detail-my-record" data-detail-part="activity"><div class="detail-section-head"><h2>감상 기록</h2><span>${logs.length ? `${logs.length}회` : ''}</span></div>${logs.length ? c.viewingHistoryHtml(record) : '<div class="activity-empty">아직 감상 기록이 없습니다.</div>'}${relationship ? `<div class="activity-relationship-summary">${relationship.rating ? `<span>현재 평점 ★ ${relationship.rating}</span>` : ''}${commentOf(relationship) ? `<span>${c.escapeHtml(commentOf(relationship))}</span>` : ''}</div>` : ''}</section>`;
}
